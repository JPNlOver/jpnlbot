...
const tools = new (require(__dirname + '/Tools'))()
...
module.exports = class IOTools {
  getImagePath (filePath) {
    const finalPath = path.join(__dirname + '/images', filePath)
  ...
  getImage (filename) {
    return new Promise((resolve, reject) => {
      const finalPath = path.join(__dirname, __dirname + '/images', filename)
  ...
  getImageFilenames (dirPath) {
    const finalDir = path.join(__dirname + '/images', dirPath)
    let filePaths = []
